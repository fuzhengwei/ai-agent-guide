...
2.对于账单明细数据量级很大（例如每月账单明细量级超过20w）的客户，通过 API 调用账单数据效率较低，建议您开通账单数据存储功能，通过存储桶中获取账单文件进行分析。[账单存储至COS桶](https://cloud.tencent.com/document/product/555/61275)
     ...
     */
    DescribeAccountWarning(req?: DescribeAccountWarningRequest, cb?: (error: string, rep: DescribeAccountWarningResponse) => void): Promise<DescribeAccountWarningResponse>;
    /**
...
1. 对于消耗明细数据量级很大（例如每月消耗明细量级超过100w）的客户，通过 API 调用明细数据会有超时风险，建议您开通消耗账单数据存储功能，通过存储桶中获取账单文件进行分析。[账单存储至COS桶](https://cloud.tencent.com/document/product/555/61275)
     ...
     * 云api设置余额告警阈值接口
     */
    ModifyAccountWarning(req: ModifyAccountWarningRequest, cb?: (error: string, rep: ModifyAccountWarningResponse) => void): Promise<ModifyAccountWarningResponse>;
}
