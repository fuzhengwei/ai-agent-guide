...
* 只能查询到最近七天（168小时）内的任务。
     ...
     * 调用该接口，用于创建Agent录制任务。
     */
    async CreateAgentRecordTask(req, cb) {
...
注意：模板 ID 为 10000 以下的为系统预置模板，不允许删除。
     ...
     * 提交创建混元3D的任务
     */
    async SubmitHunyuan3DTask(req, cb) {
...
包含 confirm（确认当前阶段）和 regenerate（重新生成当前阶段）两种动作含义。
     */
    async ModifyDocToVideoTaskStatus(req, cb) {
...
5. 美颜滤镜；
     ...
     */
    async DescribeAgentRecordTask(req, cb) {
...
直播流处理事件通知支持HTTP回调，也支持实时写入用户指定的消息队列 CMQ 中，用户从消息队列 CMQ 中获取事件通知结果，同时处理过程中存在输出文件的，会写入用户指定的输出文件的目标存储中。
     ...
     * 查询混元3D任务对应的结果
     */
    async QueryHunyuan3DTask(req, cb) {
