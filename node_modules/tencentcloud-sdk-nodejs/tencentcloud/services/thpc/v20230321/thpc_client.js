...
* 若代理未开通，EndPointReady返回false，EndPointStatus为UNKNOWN。
     */
    async DescribeClusterDedicatedProxy(req, cb) {
...
* 仅支持IDC类型集群，对非IDC集群调用将返回<code>InvalidParameterValue.ParametersNotSupported</code>。
     */
    async GenerateRegisterCommand(req, cb) {
...
* 若集群已开通代理，需先关闭代理（DisableClusterDedicatedProxy）再变更VPC绑定。
     ...
     */
    async InquirePriceCreateWorkspaces(req, cb) {
     ...
     */
    async DescribeQueueAutoScalingOverview(req, cb) {
     ...
     */
    async DescribeInstanceFamilies(req, cb) {
...
* 操作不可逆，关闭后需重新调用EnableClusterDedicatedProxy开启。
     */
    async DisableClusterDedicatedProxy(req, cb) {
...
* 单个集群中队列数量上限为10个。
     ...
     * 查询指定集群的队列弹性伸缩配置信息。
     */
    async DescribeQueueAutoScaling(req, cb) {
...
* SubnetId与VpcId需同时指定或同时不指定。若不指定，则使用集群已绑定的VPC和子网。
     */
    async EnableClusterDedicatedProxy(req, cb) {
